...
func solve(prog []uint8, index int, aBase int64) int64 {
	for curr := range int64(8) {
		a := aBase*8 + curr

		var comp computer
		comp.a = a

		if slices.Compare(comp.run(prog), prog[index:]) == 0 {
			if index == 0 {
				return a
			}
			ret := solve(prog, index-1, a)
			if ret != -1 {
				return ret
			}
		}
...
func main() {
	_, prog := parseInput(os.Stdin)
	a := solve(prog, len(prog)-1, 0)
	fmt.Println(a)
}
